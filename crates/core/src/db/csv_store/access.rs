//! Database access guards for the CSV field cache.
...
/// Ensures the SQLite database exists and contains the `csv_fields` table.
pub fn ensure_csv_cache(database_path: &Path) -> NestResult<()> {
    ...
    let exists: i32 = conn
        .query_row(
            "SELECT COUNT(*) FROM sqlite_master WHERE type = 'table' AND name = 'csv_fields'",
            [],
            |row| row.get(0),
        )
    ...
    if exists == 0 {
        return Err(NestError::data(format!(
            "database is missing csv_fields (migration {MIGRATION_ID} not applied)"
        ))
