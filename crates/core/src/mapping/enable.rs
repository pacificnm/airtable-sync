//! `mapping enable` command handler.
...
/// Cached table metadata in mapping enable output.
#[derive(Debug, Clone, PartialEq, Eq, Serialize)]
pub struct MappingEnableTableView {
...
/// JSON response for `mapping enable` with `--json`.
#[derive(Debug, Serialize)]
pub struct MappingEnableResult {
    /// Absolute path to the SQLite database file.
    pub database_path: PathBuf,
    /// Airtable base id from config.
    pub base_id: String,
    /// Cached table metadata.
    pub table: MappingEnableTableView,
    ...
    /// Whether sync was turned on (false when already enabled).
    pub enabled: bool,
}

/// Enables field synchronization for one mapped field.
pub fn enable_mapping(ctx: &AppContext, matches: &ArgMatches) -> NestResult<()> {
    ...
    let updated = schema_store
        .set_field_sync_enabled(&table.table_id, field_name, true)
        .map_err(NestError::from)?;
    ...
    print_mapping_enable_success(&result, json, quiet)
}

impl From<AirtableTableRow> for MappingEnableTableView {
...
fn print_mapping_enable_success(
    result: &MappingEnableResult,
    json: bool,
    quiet: bool,
) -> NestResult<()> {
    if json {
        let payload = serde_json::to_string_pretty(result).map_err(|error| {
            NestError::data(format!("failed to serialize mapping enable result: {error}"))
        })?;
        println!("{payload}");
        return Ok(());
    }
