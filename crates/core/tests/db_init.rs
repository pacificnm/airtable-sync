...
#[test]
fn init_creates_database_and_applies_schema() {
    let _lock = test_lock();
...
#[test]
fn init_fails_when_database_already_exists() {
    let _lock = test_lock();
    let dir = tempdir().unwrap();
    let config_path = write_valid_fixture(&dir);
    let db_path = dir.path().join("data/app.db");
...
#[test]
fn init_fails_when_config_invalid() {
    let _lock = test_lock();
    let dir = tempdir().unwrap();
    let config_path = dir.path().join("config.toml");
    fs::write(
...
#[test]
fn init_quiet_json_produces_no_extra_output() {
    let _lock = test_lock();
    let dir = tempdir().unwrap();
    let config_path = write_valid_fixture(&dir);
