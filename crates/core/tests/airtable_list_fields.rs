//! `airtable list-fields` integration tests.
...
#[tokio::test]
async fn list_fields_returns_cached_fields_after_pull_schema() {
    let _lock = test_lock();
...
#[tokio::test]
async fn list_fields_fails_for_unknown_table() {
    let _lock = test_lock();
    let server = MockServer::start().await;
    mount_meta_schema(&server).await;
...
#[test]
fn list_fields_fails_when_database_missing() {
    let _lock = test_lock();
    let dir = tempdir().unwrap();
