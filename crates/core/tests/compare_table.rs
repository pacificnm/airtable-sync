//! `compare table` integration tests.
...
async fn mount_list_records(server: &MockServer) {
    Mock::given(method("GET"))
...
fn set_mappings(config_path: &std::path::Path) {
    cli_app()
        .try_run_with([
            "airtable-sync",
            "--config",
...
#[tokio::test]
async fn compare_table_reports_matches_and_diffs() {
    let _lock = test_lock();
    let server = MockServer::start().await;
    mount_meta_schema(&server).await;
    mount_list_records(&server).await;
...
#[tokio::test]
async fn compare_table_requires_primary_key_field() {
    let _lock = test_lock();
    let server = MockServer::start().await;
    mount_meta_schema(&server).await;
