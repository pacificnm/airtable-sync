//! `airtable list-tables` integration tests.
...
#[tokio::test]
async fn list_tables_returns_cached_tables_after_pull_schema() {
    let _lock = test_lock();
...
#[tokio::test]
async fn list_tables_succeeds_with_empty_cache() {
    let _lock = test_lock();
    let dir = tempdir().unwrap();
...
#[test]
fn list_tables_fails_when_database_missing() {
    let _lock = test_lock();
    let dir = tempdir().unwrap();
    let config_path = write_fixture(&dir, "https://example.invalid/meta");
