//! `report changes` integration tests.
...
#[tokio::test]
async fn report_changes_lists_operations_after_dry_run() {
    let _lock = test_lock();
...
#[tokio::test]
async fn report_changes_filters_by_status() {
    let _lock = test_lock();
    let server = MockServer::start().await;
    mount_meta_schema(&server).await;
    mount_assets_records(&server).await;
...
#[tokio::test]
async fn report_changes_fails_without_change_plan() {
    let _lock = test_lock();
    let dir = tempdir().unwrap();
