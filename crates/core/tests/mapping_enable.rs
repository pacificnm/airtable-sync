//! `mapping enable` integration tests.
...
#[tokio::test]
async fn mapping_enable_turns_on_sync_for_mapped_field() {
    let _lock = test_lock();
...
#[tokio::test]
async fn mapping_enable_succeeds_when_already_enabled() {
    let _lock = test_lock();
    let server = MockServer::start().await;
    mount_meta_schema(&server).await;
...
#[tokio::test]
async fn mapping_enable_fails_without_csv_mapping() {
    let _lock = test_lock();
    let server = MockServer::start().await;
    mount_meta_schema(&server).await;
...
#[tokio::test]
async fn mapping_enable_rejects_computed_field() {
    let _lock = test_lock();
    let server = MockServer::start().await;
    mount_meta_schema(&server).await;
