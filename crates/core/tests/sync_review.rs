//! `sync review` and approval integration tests.
...
fn first_operation_id(db_path: &std::path::Path) -> i64 {
    let conn = Connection::open(db_path).unwrap();
    conn.query_row(
        "SELECT id FROM change_plan_operations ORDER BY id ASC LIMIT 1",
        [],
        |row| row.get(0),
    )
    .unwrap()
...
fn approved_count(db_path: &std::path::Path) -> i64 {
    let conn = Connection::open(db_path).unwrap();
    conn.query_row(
        "SELECT COUNT(*) FROM change_plan_operations WHERE status = 'approved'",
        [],
        |row| row.get(0),
    )
    .unwrap()
...
fn plan_count(db_path: &std::path::Path) -> i64 {
    let conn = Connection::open(db_path).unwrap();
...
#[tokio::test]
async fn sync_review_shows_pending_operations_after_dry_run() {
    let _lock = test_lock();
...
#[tokio::test]
async fn sync_approve_marks_one_operation_approved() {
    let _lock = test_lock();
    let server = MockServer::start().await;
    mount_meta_schema(&server).await;
    mount_assets_records(&server, 1).await;
    ...
    let operation_id = first_operation_id(&db_path);
    cli_app()
        .try_run_with([
            "airtable-sync",
            "--config",
    ...
    assert_eq!(approved_count(&db_path), 1);
}

#[tokio::test]
async fn sync_approve_all_approves_pending_operations() {
    let _lock = test_lock();
    let server = MockServer::start().await;
    mount_meta_schema(&server).await;
    mount_assets_records(&server, 1).await;
    ...
    assert!(approved_count(&db_path) >= 1);
}

#[tokio::test]
async fn second_dry_run_supersedes_previous_plan() {
    let _lock = test_lock();
    let server = MockServer::start().await;
    mount_meta_schema(&server).await;
    mount_assets_records(&server, 2).await;
    ...
    assert_eq!(plan_count(&db_path), 2);
    assert_eq!(latest_plan_status(&db_path), "draft");
}

#[tokio::test]
async fn sync_approve_rejects_denied_operation() {
    let _lock = test_lock();
    let server = MockServer::start().await;
    mount_meta_schema(&server).await;
    mount_assets_records(&server, 1).await;
...
#[tokio::test]
async fn sync_deny_all_denies_pending_operations() {
    let _lock = test_lock();
    let server = MockServer::start().await;
    mount_meta_schema(&server).await;
    mount_assets_records(&server, 1).await;
    ...
    let denied: i64 = conn
        .query_row(
            "SELECT COUNT(*) FROM change_plan_operations WHERE status = 'denied'",
            [],
            |row| row.get(0),
        )
        .unwrap();
