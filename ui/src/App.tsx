...
import { AutoMapView } from "./components/AutoMapView";
import { MappingListView } from "./components/MappingListView";
import { MappingReportView } from "./components/MappingReportView";
...
export function App() {
  ...
  const ribbon = (
    <>
      <TitleBar title="Airtable Sync" />
      <SyncRibbon
        activeTab={activeTab}
        onTabChange={setActiveTab}
        onRun={handleRun}
        onOpenDocs={() => setView("docs")}
        onOpenValidate={() => setView("validate")}
        onOpenShow={() => setView("show")}
        onOpenSettings={() => setView("settings")}
        onOpenAutoMap={() => setView("automap")}
        onOpenMappingList={() => setView("mappinglist")}
        onOpenMappingReport={() => setView("mappingreport")}
        onOpenTables={() => setView("tables")}
        onQuit={() => void quitApp()}
        busy={busy}
        activeView={view}
        activeRunKey={lastRunKey}
      />
    </>
  );
